...
let block = world.getBlock(x,y,z,dimension), target
switch(block){
case this.id:
target = this.id | SLAB
break
...
break
case this.id|STAIR:
target = this.id | CROSS
break
}
if(target) world.setBlock(x,y,z,target,false,false,false,false,dimension)
}
}
];
...
trySpawnFlow:function(x,y,z,dimension,world,level,spread){
let block = world.getBlock(x,y,z,dimension)
if(block === this.id) return //Source block
if(!blockData[block].solid && level > this.getLevel(block)){
world.spawnBlock(x,y,z,this.getLevelBlock(level),dimension,true)
spread.push(x,y,z,level)
}
return !blockData[block].solid
},
spawnFlow:function(x,y,z,dimension,world){
let spread = [x,y,z,8]
while(spread.length){
let [sx,sy,sz,level] = spread
if(level > 1 && max(abs(sx-x),abs(sz-z)) < 16 && abs(sy-y) < 64){
if(!this.trySpawnFlow(sx,sy-1,sz,dimension,world,8,spread)){
this.trySpawnFlow(sx+1,sy,sz,dimension,world,level-1,spread)
this.trySpawnFlow(sx-1,sy,sz,dimension,world,level-1,spread)
this.trySpawnFlow(sx,sy,sz+1,dimension,world,level-1,spread)
this.trySpawnFlow(sx,sy,sz-1,dimension,world,level-1,spread)
}
}
spread.splice(0,4)
...
stairBlock.drop = i
makeBlock(baseBlock.textures, shapes.flowerbed4, crossBlock, baseBlock)
crossBlock.drop = i
}
...
isState =        bin("11111110000000000000")
...
replaceBlocks(pos.prevPosCmd[0], pos.prevPosCmd[1]-1, pos.prevPosCmd[2], round(pos.x), round(pos.y-1), round(pos.z), replace, into, pos.dimension, world)
},
anonymousFunc: null
},
{
...
world.sendPlayer({type:"tp",x,y,z,dimension},pos.id)
},
anonymousFunc: null
},
{
...
if(comeCloser){
this.moveTowards(e.x, Math.min(Math.max(this.y, e.y - e.bottomH),e.y+e.topH), e.z, hRange,2.3,hRange, 3)
}
/*if(pickup){
...
this.moveTowards(e.x, e.y-e.bottomH, e.z, 7.25,7.25,7.25, 5, true)
}
return dist < 0.5
}
update(){
...
this.world.sendPlayer({type:"hit", damageType:"mobHit", username:this.name || this.defaultName, id:this.id, damage:this.attackStrength, message:msg,x:this.x,y:this.y,z:this.z}, target.id)
...
if(data.damage){
let d = data.damage
if(typeof d === "function") d = d(round(this.x),round(this.y),round(this.z),this.dimension)
...
if (x === 15 || this.getLight(x+1,y,z,2) < light) {
spread[light].push(x + this.x, y, z + this.z)
continue
}
if (x === 0 || this.getLight(x-1,y,z,2) < light) {
spread[light].push(x + this.x, y, z + this.z)
continue
}
if (z === 15 || this.getLight(x,y,z+1,2) < light) {
spread[light].push(x + this.x, y, z + this.z)
continue
}
if (z === 0 || this.getLight(x,y,z-z,2) < light) {
spread[light].push(x + this.x, y, z + this.z)
continue
}
break
...
let {blocks,tops,biomes,hasSemiTrans,minY,maxY,densityBottoms,densities,waterTops} = await doWork({generate:true,trueX,trueZ,seed:this.world.worldSeed})
this.tops.set(tops)
this.solidTops.set(tops)
this.biomes = biomes
if(hasSemiTrans){
...
var trunkHeight
var leafBottom = 8
var heightAttenuation = heightLimit/16
var branchCount
...
let maxDist = heightLimit
let minDist = maxDist * 0.75
...
for(var y=leafBottom; y<endy; y+=heightAttenuation){
for(var b=0; b<branchCount; b++){
let arr = []
let dist = random(maxDist-minDist)+minDist
let angle = random(Math.PI*2)
let angleY = map(y,leafBottom,endy,0,0.8)*Math.PI2
let sy = y-dist*sin(angleY)//(dist*branchSlope)
let sx = sy*dirx, sz = sy*dirz
let side = cos(angleY)
let ex = wx + sx + dist*0.75*sin(angle)*side
let ez = wz + sz + dist*0.75*cos(angle)*side
line3D(ex,y+ground,ez,wx+sx,sy+ground,wz+sz,arr)
branches.push(arr)
}
}
for(var y=0; y<trunkHeight; y++){
world.spawnBlock(wx+round(dirx*y),ground+y,wz+round(dirz*y),tree)
if(dying) worldGenArray.add(wx+round(dirx*y),ground+y,wz+round(dirz*y),blockIds.vine)
}
...
if(dying) worldGenArray.add(b[bi],b[bi+1],b[bi+2],blockIds.vine)
}
}
}
for(var b of branches){
...
if(n > 0.6){
n += this.world.caveNoise((lx + x)*3.3, (ly + y)*3.3, (lz + z)*3.3)*0.4
}
if (n < 1) {
...
ground = world.getSolidTop(wx+distance*dx,wz+distance*dz,type)
...
let {type, world} = this
let top = ground + floor(random(15,40))
let bottom = round(lerp(0.75,top,ground))
let tree = blockIds.spruceLog
let leaf = blockIds.spruceLeaves
for (let j = ground + 1; j < top; j++) {
this.setBlock(i, j, k, tree)
...
let size = round(l/8 + 2)
let s2 = size * size
let noiseAmount = 0.8
//Make thinner
noiseAmount += (top-j)/15
for(let x=-size; x<=size; x++){
for(let z=-size; z<=size; z++){
let n = x**2 / s2 + z**2 / s2
if(n > 1-noiseAmount){
n += this.world.caveNoise((wx+x)*3.3, j*3.3, (wz+z)*3.3)*noiseAmount
}
if (n < 1) {
world.spawnBlock(wx + x, j, wz + z, leaf, type)
}
...
if(type === "big"){
oreWeirdness = 0.02, oreW = 20, oreH = 15
...
let n = this.world.noiseProfile.noise((X+x)*oreWeirdness*4,(Y+y)*oreWeirdness/2,(Z+z)*oreWeirdness*4)
n = n - preferMiddle((y+oreH)/oreH/2, 0.1, 0,1) - ((x/oreW)**2 + (z/oreW)**2)
if(n>0){
if(type === "mossy"){
...
world.spawnBlock(X+x,Y+y,Z+z,blockIds.diorite,this.type, true)
}
}else{
...
for(let z=-size;z<=size;z++){
if(random((x/size)**2+(y/size)**2+(z/size)**2) < 0.5 && !world.getBlock(X+x,Y+y,Z+z,this.type) && world.getBlock(X+x,Y+y-1,Z+z,this.type) === blockIds.grass){
world.spawnBlock(X+x,Y+y,Z+z,blocks[floor(random(blocks.length))],this.type)
}
}
}
}
...
let {type, world} = this
let top = ground + (tall ? floor(random(14,18)) : floor(random(6,10)))
let tree = blockIds.birchLog
let leaf = blockIds.birchLeaves
//Center
for (let j = ground + 1; j <= top; j++) {
this.setBlock(i, j, k, tree | WALLPOST)
}
this.setBlock(i, ground, k, blockIds.dirt)
for(let b=0; b<16; b++){
let j = round(lerp(random()**3,top+2,ground+6))
...
spawnShrub(i,ground,k,wx,wz,berry){
let {type, world} = this
let rand = floor(random(4096))
let tree, leaf
let treeType = berry ? round(random(0,2)) : round(random(-1,0))
switch(treeType){
case -1:
...
spawnBush(i,ground,k,wx,wz,berry){
let {type, world} = this
let rand = floor(random(4096))
let tree, leaf
let treeType = berry ? round(random(0,2)) : round(random(-1,0))
...
let w = round(random(2,3)), h = round(random(1,4))
...
spawnHugeMushroom(i,ground,k,wx,wz){
let {type, world} = this
...
this.setBlock(i, ground, k, blockIds.dirt)
if(random() > 0.5){//Brown mushroom (flat)
...
flowerForestFlowers:[blockIds.dandelion,blockIds.poppy,blockIds.allium,blockIds.azureBluet,blockIds.orangeTulip,blockIds.pinkTulip,blockIds.redTulip,blockIds.whiteTulip,blockIds.oxeyeDaisy,blockIds.cornFlower,blockIds.lilyOftheValley,blockIds.pinkPetals],
...
pinkPetals:[blockIds.pinkPetals,blockIds.pinkPetals|SLAB,blockIds.pinkPetals|STAIR,blockIds.pinkPetals|CROSS],
...
fernBerries:[blockIds.fern,blockIds.sweetBerryBush|CROSS,blockIds.largeFern,blockIds.TallGrass,blockIds.TallGrass,blockIds.DoubleTallGrass],
...
emeraldOre: {biomes:["windsweptHills","grove","jaggedPeaks","meadows","frozenPeaks","stonePeaks","windsweptForest","snowySlopes","windsweptGravelHills","cherryGrove"], [blockIds.stone]:blockIds.emeraldOre,[blockIds.deepslate]:blockIds.deepslateEmeraldOre},
...
if(biome !== "frozenPeaks" && biome !== "snowySlopes" && biome !== "jaggedPeaks" && biome !== "windsweptHills" && biome !== "windsweptGravelHills") continue
...
}else if(biome === "windsweptGravelHills"){
if(steepness>2.5){
let y = ground
while(this.getBlock(i,y,k) === blockIds.grass || this.getBlock(i,y,k) === blockIds.dirt){
...
let birchAndOak, bigOak, oak, birch, tallBirch, smallJungle, bigJungle, spruce, pine, bigSpruce, bigPine, acacia, bigAcacia, bigBirch, tallSpruce, darkOak, cherry
let deadBush, cactus, sugarcane = 0.2, melonPatch, kelp, hugeMushroom, shrubBush, bush, shrubBushBerry = true
let rock = ground > 90 ? 0.25 : 0.05, rockType = ground > 90 ? "flat" : "medium"
let grassType, flowerType, bamboo, flowerDensity = 0.65, flowerNoiseSize = 0.04, grassDensity = 0.4, seagrass //flowerDensity more is less flowers
let fallenOak, fallenBirch, fallenTallBirch, fallenJungle, fallenSpruce
switch(b){
case "forest":
bush = 2, birchAndOak = 9, bigOak = 1, bigBirch = 0.5, flowerType = this.patches.forestFlowers, grassType = this.patches.grassLeaves, grassDensity = 0.15
break
case "plains":
...
break
case "birchForest":
bigBirch = 4, birch = 4, flowerType = this.patches.forestFlowers, grassType = this.patches.grass
break
case "oldBirchForest":
...
break
case "oldSpruceTaiga":
bigOak = 1, oak = 2, bush = 1, deadBush = 1, tallSpruce = 8, spruce = 0.5, bigSpruce = 0.5, pine = 0.25, bigPine = 0.125, rock = 1, rockType = "mossy", grassType = this.patches.fernGrass, grassDensity = 0.4, flowerType = this.patches.flowers
break
case "oldPineTaiga":
bigOak = 1, oak = 2, bush = 1, deadBush = 1, tallSpruce = 0.5, pine = 1, bigPine = 2, spruce = 0.25, bigSpruce = 0.125, rock = 1, rockType = "mossy", grassType = this.patches.fernGrass, grassDensity = 0.4, flowerType = this.patches.flowers
break
case "grove":
...
oak = 1, spruce = 0.666, bigSpruce = 0.3, bigOak = 0.1, flowerDensity = 0.8, rock = 1, rockType = "small", rock = 0, flowerType = this.patches.flowers, grassType = this.patches.grass, grassDensity = 0.6
break
case "windsweptHills":
rock = 1, rockType = "small", bigOak = 0.1, bigSpruce = 0.1
break
case "taiga":
spruce = 3, pine = 2, bigSpruce = 0.25, bigPine = 0.25, tallSpruce = 0.25, flowerDensity = 0.8, flowerType = this.patches.flowers, grassType = this.patches.fernBerries, grassDensity = 0.4
break
case "snowyTaiga":
spruce = 2, pine = 0.666, bigSpruce = 0.25, bigPine = 0.25, tallSpruce = 0.25, flowerDensity = 0.8, flowerType = this.patches.flowers, grassType = this.patches.fernGrass, grassDensity = 0.4
break
case "snowyPlains":
...
case "savanna":
case "savannaPlateau":
acacia = 0.5, bigAcacia = 0.05, oak = 0.1, grassType = this.patches.tallGrass, flowerType = this.patches.flowers
break
case "shrubland":
rock = 0.5, rockType = "small", shrubBush = 8, spruce = 0.25, oak = 0.5, grassType = this.patches.tallGrass, grassDensity = 0.2, flowerType = this.patches.flowers
break
case "sparseForest":
bush = 2, tallBirch = 1, bigOak = 3, bigBirch = 0.125, spruce = 0.25, bigSpruce = 0.0625, tallSpruce = 0.1, flowerType = this.patches.forestFlowers, grassType = this.patches.mostThickLeaves, grassDensity = 0.1
break
case "alpineTundra":
...
break
case "rockyPlains":
rock = 0.175, rockType = "big", bigOak = 0.04, bigSpruce = 0.04, grassType = this.patches.tallGrass, grassDensity = 0.2
break
case "windsweptSavanna":
acacia = 0.5, grassType = this.patches.grass, flowerType = this.patches.flowers
break
case "grassyForest":
birchAndOak = 9, bigOak = 0.5, flowerType = this.patches.forestFlowers, grassType = this.patches.grass
break
case "cherryGrove":
...
break
case "dryShrubland":
shrubBush = 6, shrubBushBerry = false, deadBush = 1, cactus = 1, rock = 0
break
case "snowySlopes":
case "frozenPeaks":
case "jaggedPeaks":
...
if(!aboveGround && birchAndOak && under && random(256) < birchAndOak){
this.spawnSmallTree(i,ground,k,wx,wz,random() > 0.8, random() > 0.9)
}
if(!aboveGround && oak && under && random(256) < oak){
this.spawnSmallTree(i,ground,k,wx,wz, false)
}
if(!aboveGround && birch && under && random(256) < birch){
this.spawnSmallTree(i,ground,k,wx,wz,true,false,true)
}
if(!aboveGround && bigOak && under && random(256) < bigOak){
...
if(!aboveGround && bigBirch && under && random(256) < bigBirch){
this.spawnBigBirch(i,ground,k,wx,wz)
}
if(!aboveGround && tallBirch && under && random(256) < tallBirch){
this.spawnBigBirch(i,ground,k,wx,wz,true)
}
if(!aboveGround && shrubBush && under && random(256) < shrubBush){
if(random() > 0.5){
this.spawnBush(i,ground,k,wx,wz,shrubBushBerry)
}else{
this.spawnShrub(i,ground,k,wx,wz,shrubBushBerry)
}
}
if(!aboveGround && bush && under && random(256) < bush){
this.spawnBush(i,ground,k,wx,wz)
}
if(!aboveGround && tallSpruce && under && random(256) < tallSpruce){
this.spawnTallSpruce(i,ground,k,wx,wz)
}
if(!aboveGround && darkOak && under && random(256) < darkOak){
this.spawnDarkOak(i,ground,k,wx,wz)
}
if(!aboveGround && hugeMushroom && under && random(256) < hugeMushroom){
this.spawnHugeMushroom(i,ground,k,wx,wz)
}
if(!aboveGround && cherry && under && random(256) < cherry){
...
if(!aboveGround && flowerType && blockData[under].type === "ground" && random(mapClamped(noiseProfile.noise(wx*0.1,2,wz*0.1),0.3,0.7)) > flowerDensity){
let f = flowerType[round(mapClamped(noiseProfile.noise(wx*flowerNoiseSize,3,wz*flowerNoiseSize),0.3,0.7)*(flowerType.length-1))]
...
for (let i = 0; i < 16; i++) {
for (let k = 0; k < 16; k++) {
let top = max(this.solidTops[k*16+i],63)
...
if(top){
if(getBiomeTemperature(biomes[this.biomes[k*16+i]],top) <= 0.15){
this.setBlock(i,top,k,blockIds.ice)
}else{
blockData[blockIds.Water].spawnFlow(i+this.x,top,k+this.z,type,world)
}
}
}
}
...
let currentRandoms = [], currentRandom
let randoms = {}
const randomSeed = seed => {
currentRandoms.length = 0
currentRandoms.push(new Marsaglia(seed))
for(let i=0; i<4; i++) currentRandoms.push(new Marsaglia(hash(seed,i) * 210000000))
...
iceSpikes:[0,0.1],
snowyPlains:[0,0.1, [["Rabbit",1,1],["PolarBear",1,1]],hostileMobs],
grove:[-0.2,0.5, taigaMobs,hostileMobs],
alpineTundra:[0.18,0.25,taigaMobs,hostileMobs],
snowyTaiga:[-0.5,0.1, taigaMobs,hostileMobs],
taiga:[0.25,0.8, taigaMobs,hostileMobs],
shrubland:[0.85,0.4, grassMobs,hostileMobs],
meadows:[0.3,0.6, meadowMobs,hostileMobs],
forest:[0.7,0.8, grassMobs,hostileMobs],
sparseForest:[0.6,0.6, grassMobs,hostileMobs],
...
//lushHills:[0.2,0.8, grassMobs,hostileMobs],
...
rockyPlains:[0.8,0.4, grassMobs,hostileMobs],
grassyForest:[0.7,0.8, grassMobs,hostileMobs],
mushroomFields:[0.9,1, ["Mooshroom",4,4]],
cherryGrove:[0.3,0.6, meadowMobs,hostileMobs],
dryShrubland:[1,0.2],
trashland:[0.25,0],
...
class World{
constructor({
trees = true, caves = true, superflat = false,
preBeta, survival = false, cheats = true,
operators = [], settings = defaultWorldSettings
} = {}) {
this.version = preBeta ? "Alpha 1.0.5" : version
...
this.survival = survival //game mode on join
...
this.spawnPoint.y = this.superflat ? 6 : round(this.noiseProfile.noise(this.spawnPoint.x * generator.smooth, this.spawnPoint.z * generator.smooth) * generator.height) + 2 + generator.extra
}
}
...
blockData[cblock].addPetal(x,y,z,dimension,world)
}
}else{
...
} else if (force || !blockData[chunk.getBlock(x & 15, y, z & 15)].solid) {
...
break
case "TextDisplay":
ent = new ent(x,y,z,text,size,color,background)
default:
break
}
if(!ent) return
...
if(!chunk.generated){
chunk.generate()
}else if (!chunk.caves && !this.usePreBeta) {
chunk.carveCaves()
}else{
...
if (!chunk.caves && this.usePreBeta) {
chunk.carveCaves()
...
if(loadFrom[x+","+z]) this.loadQueue.push(chunk)
...
changes = true
let x = (i >> 8) + section.x
let y = (i >> 4 & 15) + section.y - (this.usePreBeta?0:minHeight)
...
if(!this.usePreBeta) bab.add(this.survival?(this.survival==="hardcore"?2:1):0,2).add(this.cheats?1:0,1)
return bab.array
}
...
if(survivLength) inv.survivStr = reader.readToArrayBits(survivLength)
