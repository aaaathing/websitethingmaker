...
if(level > 1 && max(abs(sx-x),abs(sy-y),abs(sz-z)) < 16){
...
var maxDist = heightLimit
var minDist = maxDist * 0.75
...
let side = cos(angleY)
let ex = wx + dist*0.75*sin(angle)*side
let ez = wz + dist*0.75*cos(angle)*side
...
if(dying) worldGenArray.add(wx,ground+y,wz,blockIds.vine)
}
...
n += this.world.caveNoise((wx+x2)*1.5, (top+y2)*1.5, (wz+z2)*1.5)*1.5
if (n < 1) {
world.spawnBlock(wx+x2, top+y2, wz+z2, leaf, type)
}
}
...
spawnShrub(i,ground,k,wx,wz){
let {type, world} = this
let rand = floor(random(4096))
let tree, leaf
let treeType = floor(random(3))
switch(treeType){
case 0:
...
let rand = floor(random(4096))
let tree, leaf
let treeType = berry ? floor(random(3)) : 0
switch(treeType){
case 0:
tree = blockIds.oakLog, leaf = blockIds.oakLeaves
break
...
grassLeaves:[blockIds.driedOakLeaves,blockIds.driedBirchLeaves,blockIds.TallGrass,blockIds.TallGrass,blockIds.TallGrass],
...
if(biome !== "frozenPeaks" && biome !== "snowySlopes" && biome !== "jaggedPeaks") continue
...
let birchAndOak, bigOak, oak, birch, tallBirch, smallJungle, bigJungle, spruce, pine, bigSpruce, bigPine, acacia, bigAcacia, bigBirch, shrubBush, bush, tallSpruce, darkOak
...
break
case "shrubland":
rock = 0.1, rockType = "big", shrubBush = 8, grassType = this.patches.tallGrass, grassDensity = 0.2, flowerType = this.patches.flowers
break
case "sparseForest":
bush = 2, birchAndOak = 1, bigOak = 0.125, bigBirch = 0.125, spruce = 0.25, bigSpruce = 0.0625, tallSpruce = 0.1, flowerType = this.patches.forestFlowers, grassType = this.patches.grass, grassDensity = 0.3
break
case "alpineTundra":
...
if(random() > 0.5){
this.spawnBush(i,ground,k,wx,wz,true)
}else{
this.spawnShrub(i,ground,k,wx,wz)
}
}
...
if(aboveGround === blockIds.Water && random() < (8-Math.max((60-ground)/2/*deeper water is less disks*/,0))/256){
let disk = waterTop ? floor((1-(1-random())**2)*5/*make mud more common*/) : floor(random(4)), block, underBlock, replace
switch(disk){
...
if(!blockData[block].slide && getBiomeTemperature(biomes[this.biomes[k*16+i]],top) <= 0.15){
if(block === blockIds.Water){
...
let block = blockData[chunk.getBlock(cx, i, cz)]
if (block.transparent) {
chunk.setLight(cx, i, cz, 0, 2)
chunk.setLight(cx, i, cz, 0, 0)
spread[center].push(x, i, z)
} else {
break
}
}
}
chunk.unSpreadLight(spread[center], center - 1, respread)
if(blockData[this.getBlock(x,y,z,dimension)].transparent){
//Spread light if block decreases light but does not block it
spread[center].length = 0
let light = centerExposed
for (let i = y; i > minHeight; i--) {
let block = blockData[chunk.getBlock(cx, i, cz)]
if (block.transparent) {
if(block.decreaseLight){
...
chunk.setLight(cx, i, cz, light, 2)
up = this.getLight(cx, i+1, cz, 0, dimension)
down = this.getLight(cx, i-1, cz, 0, dimension)
north = this.getLight(cx, i, cz+1, 0, dimension)
south = this.getLight(cx, i, cz-1, 0, dimension)
east = this.getLight(cx+1, i, cz, 0, dimension)
west = this.getLight(cx-1, i, cz, 0, dimension)
let maxLight = max(max(up, down, north, south, east, west)-1, light, 0)
if(maxLight > chunk.getLight(cx, i, cz, 0)) chunk.setLight(cx, i, cz, maxLight, 0)
spread[light].push(x, i, z)
} else {
break
}
