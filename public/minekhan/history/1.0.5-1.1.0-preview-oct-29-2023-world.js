...
item: true,
serveronuse: (x,y,z, block, dimension,world,face) => {
...
let i=wx, j=wy, k=wz
let ground = wy
let top = ground + floor(4.5 + (Math.random()*2.5) )
...
Name:"Bone \"Meal\"",
item:true,
serveronuse:function(x,y,z, block,dimension, world,face){
...
[customFace(0,10,16, 16,10,16, 16,0,8, 0,0,8, 0,0),customFace(16,10,16, 0,10,16, 0,0,8, 16,0,8, 16,0, -16)],
[customFace(0,10,0, 0,10,16, 8,0,16, 8,0,0, 0,0),customFace(0,10,16, 0,10,0, 8,0,0, 8,0,16, 16,0, -16)],
[customFace(16,10,16, 16,10,0, 8,0,0, 8,0,16, 0,0),customFace(16,10,0, 16,10,16, 8,0,16, 8,0,0, 16,0, -16)]
],
cull: {
top: 0,
bottom: 0,
...
if ((this.blocks[rnd] & isCube) === blockIds.grass || (this.blocks[rnd] & isCube) === blockIds.mycelium) {
// Spread grass
let x = (rnd >> 8) + this.x
...
this.world.setBlock(x, y, z, (this.blocks[rnd] & isCube) | (spreadTo & (~isCube)), false,false,false,false, this.type)
}
} else if((this.blocks[rnd] & isCube) === blockIds.crimsonNylium || (this.blocks[rnd] & isCube) === blockIds.warpedNylium){
// Spread nylium
let x = (rnd >> 8) + this.x
let y = (rnd >> 4 & 15) + this.y
let z = (rnd & 15) + this.z
...
if(dying) worldGenArray.add(trunk[i],trunk[i+1],trunk[i+2],blockIds.vine)
}
...
n += this.world.caveNoise(wx+x*mult, (j+n)*0.5, wz+z*mult)*noiseAmount
if (n < 1) {
world.spawnBlock(wx + x, j, wz + z, leaf, type)
}
}
...
world.spawnBlock(X+x,Y+y,Z+z,blockIds.limestone,this.type, true)
}
}else{
...
blockData[blockIds.Lava].spawnFlow(this.x+i,y,this.z+k,type,world)
}
}
let l
if(random() < 0.005){
...
if(biome === "frozenPeaks" || biome === "snowySlopes" || biome === "jaggedPeaks" || biome === "windsweptHills" || biome === "windsweptGravelHills"){
...
let deadBush, cactus, sugarcane = 0.2, melonPatch, kelp, hugeMushroom, shrubBush, bush, shrubBushBerry = true, lilyPad
let rock = ground > 90 ? 0.15 : 0.05, rockType = ground > 90 ? "flat" : "medium"
...
let fallenOak, fallenBirch, fallenTallBirch, fallenCocoa, fallenSpruce
switch(b){
case "forest":
...
break
case "meadows":
birchAndOak = 0.02, bigOak = 0.02, flowerDensity = 0.6, rock = 0, flowerType = this.patches.meadowFlowers, grassType = this.patches.grassAndRareTallGrass, grassDensity = 0.1
break
case "birchForest":
...
break
case "sparseJungle":
bamboo = 0.25, cocoa = 2, bigOak = 0.25, melonPatch = true, grassType = this.patches.grass
break
case "jungle":
bamboo = 0.25, cocoa = 8, bigJungle = 1, bigOak = 0.5, melonPatch = true, grassType = this.patches.jungle
break
case "bambooJungle":
...
break
case "shrubland":
rock = 0.1, rockType = "small", shrubBush = 8, spruce = 0.25, oak = 0.5, grassType = this.patches.tallGrass, grassDensity = 0.2, flowerType = this.patches.flowers
break
case "sparseForest":
...
break
case "dryShrubland":
shrubBush = 6, shrubBushBerry = false, deadBush = 1, cactus = 1, rock = 0, sugarcane = 0.4
break
case "snowySlopes":
case "frozenPeaks":
case "jaggedPeaks":
...
break
case "swamp":
swampOak = 1, lilyPad = 12, flowerType = this.patches.blueOrchids, flowerDensity = 0.75, grassType = this.patches.grass, deadBush = 1, hugeMushroom = 0.25
break
}
...
if(random()>0.5) this.spawnCocoaTree(i,ground,k,wx,wz)
...
blockData[block].spawnFlow(wx,y,wz,type,world)
}
}
//place snow
for (let i = 0; i < 16; i++) {
...
this.setBlock(i,top,k,blockIds.Water)
...
const hostileMobs = [["Zombie",4,4],["Skeleton",4,4],["Creeper",4,4],["Enderman",4,4],["Spider",4,4]]
const meadowMobs = [["Rabbit",2,6],["Sheep",2,4],["Donkey",1,2]]
const swampMobs = [...defaultMobs,["Frog",2,5]]
const swampHostileMobs = [...hostileMobs,["Slime",4,4],["Witch",1,1]]
...
alpineHills:[0.18,0.25,taigaMobs,hostileMobs],
...
serverChangeBlock(x,y,z,dimension,action,block,p,face){
const {holding} = p
const world = this
if(action === "click"){
...
let cblock = this.getBlock(x,y,z,dimension)
let data = blockData[holding]
if(data.serveronuse) data.serveronuse(x,y,z,cblock,dimension,this,face)
}else{
...
bab.add(sx, 16).add(sy, 8).add(sz, 16).add(dimension,3)
// Determine the most compact orientation by checking all 6!
let bestBAB = null
for (let i = 0; i < 6; i++) {
let bab = new BitArrayBuilder()
...
world.serverChangeBlock(data.x,data.y,data.z,data.dimension,data.action,data.block,data.p,data.face)
...
if(c.onGetInv) c.onGetInv()//prevent promise not resolving
}
