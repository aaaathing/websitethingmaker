...
world.spawnBlock(x,y,z,blockIds.Water,dimension)
let spread = [x,y,z,8]
while(spread.length){
let [sx,sy,sz,level] = spread
if(max(abs(sx-x),abs(sy-y),abs(sz-z)) < 16 && level > 1){
...
const {world} = this
let oreWeirdness,oreSize
if(type === "big"){
oreWeirdness = 0.02,oreSize = 20
}else if(type === "mossy" || type === "small"){
oreWeirdness = 0.1,oreSize = 3
}else if(type === "medium"){
oreWeirdness = 0.04,oreSize = 8
}
for(let x=-oreSize;x<oreSize;x++){
...
n = n - preferMiddle((y+oreSize)/oreSize/2, 0.1, 0,1) - ((x/oreSize)**2 + (z/oreSize)**2)
if(n>0){
if(type === "mossy"){
...
let w = round(random(1,2)), h = round(random(1,4))
...
switch(b){
case "forest":
bush = 2, birchAndOak = 4, bigOak = 1, bigBirch = 0.5, flowerType = this.patches.forestFlowers, grassType = this.patches.grassLeaves, grassDensity = 0.15
break
case "plains":
...
break
case "taiga":
spruce = 2, pine = 0.666, bigSpruce = 0.25, bigPine = 0.25, tallSpruce = 0.25, flowerDensity = 0.8, flowerType = this.patches.flowers, grassType = this.patches.fernBerries, grassDensity = 0.4
break
case "snowyTaiga":
